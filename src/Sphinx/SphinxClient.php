...
class SphinxClient
{
     ...
     * @throws \InvalidArgumentException When attribute name or value array is invalid
     */
    public function setFilterString($attribute, $values, $exclude = false)
    {
        if (!is_string($attribute)) {
            throw new \InvalidArgumentException('Attribute name must be a string.');
        }
        ...
        if (!is_string($values)) {
            throw new \InvalidArgumentException('Values array must be a string.');
        }
        ...
        foreach ($this->filters as $filter) {
            ...
            switch ($filter['type']) {
                ...
                case self::SPH_FILTER_STRING:
                    $req .= pack ('N', strlen($filter['values']) ) . $filter['values'];
                    break;
                default:
